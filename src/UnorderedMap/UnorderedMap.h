...
class SimpleUnorderedMap {
 public:
	//todo: делать рехеш при маленьком лоад факторе, чтобы итерирование работало за правильную асимптотику;
	...
	class IteratorImpl : public std::iterator<std::input_iterator_tag, U> {
	 public:
		...
		bool operator==(const IteratorImpl &another) const {
			return (title_iter_ == another.title_iter_) && (list_iter_ == another.list_iter_);
		}
	 ...
	 private:
		...
		IteratorImpl(VectorIter title_iter, VectorIter end_iter, ListIter list_iter) : title_iter_(std::move(title_iter)), end_iter_(std::move(end_iter)), list_iter_(std::move(list_iter)) {
		}
	...
	using Iterator = IteratorImpl<std::pair<const Key, Value>, typename Vector<List<std::pair<const Key, Value>>>::iterator, typename List<std::pair<const Key, Value>>::Iterator >;
	using ConstIterator = IteratorImpl<const std::pair<const Key, Value>, typename Vector<List<std::pair<const Key, Value>>>::const_iterator , typename List<std::pair<const Key, Value>>::ConstIterator >;
	...
	std::pair<Iterator, bool> insert(std::pair<const Key, Value> &&new_item) {
		...
		++size_;
		return std::pair{Iterator(title_iter, data_.end(), title_iter->begin()), true};
	}

	std::pair<Iterator, bool> insert(const std::pair<const Key, Value> &new_item) {
		CheckReHash();
	...
	ConstIterator end() const {
		return ConstIterator(data_.end(), data_.end(), typename List<std::pair<const Key, Value>>::ConstIterator());
	}

	Iterator end() {
		return Iterator(data_.end(), data_.end(), typename List<std::pair<const Key, Value>>::Iterator());
	}
 ...
 private:
	typename List<std::pair<const Key, Value>>::Iterator FindItem(typename Vector<List<std::pair<const Key, Value>>>::iterator title_iter, const Key &item) {
		for (auto list_iter = title_iter->begin(); list_iter != title_iter->end(); ++list_iter) {
	...
	typename List<std::pair<const Key, Value>>::ConstIterator FindItem(typename Vector<List<std::pair<const Key, Value>>>::const_iterator title_iter, const Key &item) const {
		for (auto list_iter = title_iter->begin(); list_iter != title_iter->end(); ++list_iter) {
			if (list_iter->GetItem().first == item) {
				return list_iter;
			}
	...
	typename Vector<List<std::pair<const Key, Value>>>::iterator FindTitle(const Key& item) {
		return hash_(item) % data_.size() + data_.begin();
	}

	typename Vector<List<std::pair<const Key, Value>>>::const_iterator FindTitle(const Key &item) const {
		return hash_(item) % data_.size() + data_.begin();
	}

	std::pair< typename Vector<List<std::pair<const Key, Value>>>::const_iterator, typename List<std::pair<const Key, Value>>::ConstIterator> FindTitleItem(const Key &item) const {
		auto title_iter = FindTitle(item);
		auto item_iter = FindItem(title_iter, item);
		return std::pair{title_iter, item_iter};
	}

	std::pair<typename Vector<List<std::pair<const Key, Value>>>::iterator, typename List<std::pair<const Key, Value>>::Iterator> FindTitleItem(const Key &item) {
		auto title_iter = FindTitle(item);
		auto item_iter = FindItem(title_iter, item);
		return std::pair{title_iter, item_iter};
	}
