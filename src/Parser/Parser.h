...
class Parser {
 public:
	...
	void AddDelimiter(char delimiter) {
		delimiters_.insert({delimiter, Unit()});
	}
 ...
 private:
	void BaseInitialize() {
		{
			AddToken({.type_ = BaseTokenTypes::LBRACE, .str_ = "(", .priority_ = 0, .operands_number_ = 0, .is_function = false});
			AddToken({.type_ = BaseTokenTypes::RBRACE, .str_ = ")", .priority_ = 0, .operands_number_ = 0, .is_function = false});

			AddToken({.type_ = BaseTokenTypes::PLUS, .str_ = "+", .priority_ = 1, .operands_number_ = 2, .is_function = false});
			AddToken({.type_ = BaseTokenTypes::MINUS, .str_ = "-", .priority_ = 1, .operands_number_ = 2, .is_function = false});

			AddToken({.type_ = BaseTokenTypes::MULT, .str_ = "*", .priority_ = 2, .operands_number_ = 2, .is_function = false});
			AddToken({.type_ = BaseTokenTypes::DIV, .str_ = "/", .priority_ = 2, .operands_number_ = 2, .is_function = false});

			AddToken({.type_ = BaseTokenTypes::POW, .str_ = "^", .priority_ = 3, .operands_number_ = 2, .is_function = false});
		}
	...
	int MoveTokenFromStack() {
		...
		ParseTree::Node::Ptr new_node = std::make_shared<ParseTree::Node>(token_stack_.back());
		token_stack_.pop_back();
	...
	int ProcessToken(TokenRef token_ref) {
		...
		if (token_ref->priority_ == 0) {
			...
			while (!token_stack_.empty() && token_ref->priority_ <= token_stack_.back()->priority_) {
	...
	std::optional<TokenRef> TryNumberOrVariableIteration() {
		...
		while (position_ < expr_.size() && '0' <= expr_[position_] && expr_[position_] <= '9') {
			partial_token += expr_[position_++];
		}

		if (partial_token.empty()) {
			type = BaseTokenTypes::VARIABLE;
			while (position_ < expr_.size() && 'a' <= expr_[position_] && expr_[position_] <= 'z') {
				partial_token += expr_[position_++];
			}
		}
		...
		return AddToken({.type_ = type, .str_ = partial_token, .priority_ = 0, .operands_number_ = 0, .is_function = false});
	}

	std::optional<TokenRef> Iterate(bool &error) {
		String partial_token;
		while (position_ < expr_.size() && delimiters_.find(expr_[position_]) != delimiters_.end()) {
			++position_;
		}
