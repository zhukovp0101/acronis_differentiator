...
#define PLUS(expr_1, expr_2) \
	expr_1 + "+" + expr_2

#define MINUS(expr_1, expr_2) \
	expr_1 + "-" + expr_2

#define MULT(expr_1, expr_2) \
	expr_1 + "*" + expr_2

#define DIV(expr_1, expr_2) \
	expr_1 + "/" + expr_2

#define LaTeXDIV(expr_1, expr_2) \
	"\\frac{" + expr_1 + "}{" + expr_2 + "}"
...
class Formula {
 public:
	...
	String ToString() {
		...
		StringTree string_tree = StringTree::CreateLike(tree_, [this](const Parser::ParseTree::PostOrderIterator &formula_iter, StringTree::PostOrderIterator &string_iter) {
			switch (formula_iter->value_->type_) {
				case Parser::BaseTokenTypes::PLUS: {
					...
					string_iter->value_.expr_ = PLUS(left->value_.expr_, right->value_.expr_);
				...
				case Parser::BaseTokenTypes::MINUS: {
					...
					string_iter->value_.expr_ = MINUS(left->value_.expr_, right->value_.expr_);
					string_iter->value_.is_simple_ = false;
				} break;
				...
				case Parser::BaseTokenTypes::MULT: {
					...
					string_iter->value_.expr_ = MULT(OptimizeBraced(left->value_), OptimizeBraced(right->value_));
				...
				case Parser::BaseTokenTypes::DIV: {
					...
					string_iter->value_.expr_ = DIV(OptimizeBraced(left->value_), OptimizeBraced(right->value_));
					string_iter->value_.is_simple_ = true;
				} break;
				...
				case Parser::BaseTokenTypes::POW: {
					...
					string_iter->value_.expr_ = POW(OptimizeBraced(left->value_), OptimizeBraced(right->value_));
					string_iter->value_.is_simple_ = true;
				} break;
	...
	Formula At(const UnorderedMap<String, String> &variables) {
		auto tree = Parser::ParseTree::CreateLike(tree_, [this, &variables](const Parser::ParseTree::PostOrderIterator &formula_iter, Parser::ParseTree::PostOrderIterator &mapped_formula_iter) {
			switch (formula_iter->value_->type_) {
				case Parser::BaseTokenTypes::VARIABLE: {
					...
					if (var_iter != variables.end()) {
						mapped_formula_iter->value_ = parser_.AddToken({.type_ = Parser::BaseTokenTypes::NUMBER, .str_ = var_iter->second, .priority_ = 0, .operands_number_ = 0, .is_function = false});
					}
				} break;
	...
	const Parser::ParseTree &GetTree() const {
		return tree_;
	}
		...
		for (auto &&node = tree_.begin(); node != tree_.end(); ++node) {
			switch (node->value_->type_) {
				case Parser::BaseTokenTypes::PLUS: {
					...
					if (left->value_->type_ == Parser::BaseTokenTypes::NUMBER && right->value_->type_ == Parser::BaseTokenTypes::NUMBER) {

						auto result = HandleNumbers(left->value_->str_, right->value_->str_, Parser::BaseTokenTypes::PLUS);
						auto token = parser_.AddToken({.type_ = Parser::BaseTokenTypes::NUMBER, .str_ = std::move(result), .priority_ = 0, .operands_number_ = 0, .is_function = false});
				...
				case Parser::BaseTokenTypes::MINUS: {
					...
					//					if (left->value_->str_ == "0") {
					//						node.value_ = right->value_;
					//						node->children_ = right->children_;
					//						break;
					//					}
					...
					if (left->value_->type_ == Parser::BaseTokenTypes::NUMBER && right->value_->type_ == Parser::BaseTokenTypes::NUMBER) {
						auto result = HandleNumbers(left->value_->str_, right->value_->str_, Parser::BaseTokenTypes::MINUS);
						auto token = parser_.AddToken({.type_ = Parser::BaseTokenTypes::NUMBER, .str_ = std::move(result), .priority_ = 0, .operands_number_ = 0, .is_function = false});
						tree_.Replace(node, std::make_shared<Parser::ParseTree::Node>(token));
					}
				} break;
				...
				case Parser::BaseTokenTypes::MULT: {
					...
					if (left->value_->type_ == Parser::BaseTokenTypes::NUMBER && right->value_->type_ == Parser::BaseTokenTypes::NUMBER) {
						auto result = HandleNumbers(left->value_->str_, right->value_->str_, Parser::BaseTokenTypes::MULT);
						auto token = parser_.AddToken({.type_ = Parser::BaseTokenTypes::NUMBER, .str_ = std::move(result), .priority_ = 0, .operands_number_ = 0, .is_function = false});
						tree_.Replace(node, std::make_shared<Parser::ParseTree::Node>(token));
					}
				} break;
				...
				case Parser::BaseTokenTypes::DIV: {
					...
					if (left->value_->type_ == Parser::BaseTokenTypes::NUMBER && right->value_->type_ == Parser::BaseTokenTypes::NUMBER) {
						auto result = HandleNumbers(left->value_->str_, right->value_->str_, Parser::BaseTokenTypes::DIV);
						auto token = parser_.AddToken({.type_ = Parser::BaseTokenTypes::NUMBER, .str_ = std::move(result), .priority_ = 0, .operands_number_ = 0, .is_function = false});
						tree_.Replace(node, std::make_shared<Parser::ParseTree::Node>(token));
					}
				} break;
				...
				case Parser::BaseTokenTypes::POW: {
					...
					if (right->value_->str_ == "0") {
						auto token = parser_.AddToken({.type_ = Parser::BaseTokenTypes::NUMBER, .str_ = "1", .priority_ = 0, .operands_number_ = 0, .is_function = false});
						tree_.Replace(node, std::make_shared<Parser::ParseTree::Node>(token));
						break;
					}

					if (left->value_->type_ == Parser::BaseTokenTypes::NUMBER && right->value_->type_ == Parser::BaseTokenTypes::NUMBER) {
						auto result = HandleNumbers(left->value_->str_, right->value_->str_, Parser::BaseTokenTypes::POW);
						auto token = parser_.AddToken({.type_ = Parser::BaseTokenTypes::NUMBER, .str_ = std::move(result), .priority_ = 0, .operands_number_ = 0, .is_function = false});
						tree_.Replace(node, std::make_shared<Parser::ParseTree::Node>(token));
					}
				} break;
 ...
 private:
	...
	static String HandleNumbers(const String &left_str, const String &right_str, int operation) {
	...
	String GetLaTeX() {
		...
		LaTeXTree latex_tree = LaTeXTree::CreateLike(tree_, [this](const Parser::ParseTree::PostOrderIterator &formula_iter, LaTeXTree::PostOrderIterator &latex_iter) {
			switch (formula_iter->value_->type_) {
				case Parser::BaseTokenTypes::PLUS: {
					const auto &left = latex_iter->children_[0];
					const auto &right = latex_iter->children_[1];

					latex_iter->value_.expr_ = PLUS(left->value_.expr_, right->value_.expr_);
				...
				case Parser::BaseTokenTypes::MINUS: {
					...
					latex_iter->value_.expr_ = MINUS(left->value_.expr_, LaTeXBraced(right->value_));
					latex_iter->value_.is_simple_ = false;
				} break;
				...
				case Parser::BaseTokenTypes::MULT: {
					...
					latex_iter->value_.expr_ = MULT(LaTeXBraced(left->value_), LaTeXBraced(right->value_));
				...
				case Parser::BaseTokenTypes::DIV: {
					...
					latex_iter->value_.expr_ = LaTeXDIV(left->value_.expr_, right->value_.expr_);
					latex_iter->value_.is_simple_ = true;
				} break;
				...
				case Parser::BaseTokenTypes::POW: {
					...
					latex_iter->value_.expr_ = POW(LaTeXBraced(left->value_), LaTeXBraced(right->value_));
					latex_iter->value_.is_simple_ = true;
				} break;
...
class Differentiator {
 public:
	...
	Formula Differentiate(const String &expr, String variable) {
		...
		tree_ = Tree<NodeState>::CreateLike(formula.GetTree(),
		                                    [this](const Parser::ParseTree::PostOrderIterator &expr_iter, Tree<NodeState>::PostOrderIterator &diff_iter) {
 ...
 private:
	...
	void ProcessNode(Tree<NodeState>::PostOrderIterator &cur_iter, const Parser::ParseTree::PostOrderIterator &expr_iter) {
		...
		switch (expr_iter->value_->type_) {
			...
			case Parser::BaseTokenTypes::DIV: {
				...
				current.diff_ = DIV(Braced(MINUS(MULT(Braced(left.diff_), Braced(right.normal_)),
				                                 MULT(Braced(right.diff_), Braced(left.normal_)))),
			...
			case Parser::BaseTokenTypes::POW: {
				...
				current.diff_ = MULT(POW(Braced(left.normal_), Braced(MINUS(right.normal_, "1"))),
				                     Braced(PLUS(MULT(Braced(right.normal_),
				                                      Braced(left.diff_)),
				                                 MULT(Braced(left.normal_),
				                                      MULT("log(" + left.normal_ + ")",
				                                           Braced(right.diff_))))));
