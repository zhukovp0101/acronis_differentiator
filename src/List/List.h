...
class List {
 public:
  struct ListNode {
    ...
    void LinkBefore(Ptr &&next) { next_ = std::move(next); }

    bool IsLinked() const { return next_ != nullptr; }

    T &GetItem() { return data_; }

    const T &GetItem() const { return data_; }
  ...
  class IteratorImpl : public std::iterator<std::input_iterator_tag, U> {
   public:
    ...
    typename IteratorImpl::reference operator*() const { return *current_; }

    typename IteratorImpl::pointer operator->() const { return current_; }

   private:
    friend List;

    explicit IteratorImpl(typename IteratorImpl::pointer begin)
        : current_(begin) {}
